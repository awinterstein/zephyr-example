...
West command for running clang-format on all source code files.'''
...
class ClangFormat(WestCommand):
    ...
    def do_run(self, args, unknown_args):
        ...
        # Remove last directories from script file directory and add src directory
        source_dir = os.path.join(os.path.split((os.path.split(script_dir)[0]))[0], 'src')

        # Check whether the source directory exists
        if not os.path.isdir(source_dir):
            print(f"Given source directory '{source_dir}' does not exist.", file=sys.stderr)
            sys.exit(1)

        # Find the source code files (*.h, *.c, *.hpp, *.cpp,)
        files = []
        files.extend(glob(source_dir + '/**/*.h', recursive=True))
        files.extend(glob(source_dir + '/**/*.c', recursive=True))
        files.extend(glob(source_dir + '/**/*.hpp', recursive=True))
        files.extend(glob(source_dir + '/**/*.cpp', recursive=True))
